...
[openSUSE Leap 15.6](https://doc.opensuse.org/release-notes/x86_64/openSUSE/Leap/15.6/) (Linux) for integration testing.
...
2. Clone or pull the latest changes from the [`foundata/oci-opensuseleap156-itt` git repository](https://github.com/foundata/oci-opensuseleap156-itt).
3. Change into the directory and execute the [build command](https://docs.podman.io/en/latest/markdown/podman-build.1.html):
   ```bash
   podman build -t opensuseleap156-itt .
   ```
...
2. Use the image you built earlier or pull the image from a registry:
   - [Quay](https://quay.io/repository/foundata/opensuseleap156-itt):
     ...
     ```
   - [Docker Hub](https://hub.docker.com/r/foundata/opensuseleap156-itt):
     ```bash
     podman pull docker.io/foundata/opensuseleap156-itt:latest
     ```
3. Run a container from the image:
   ```bash
   podman run --detach --systemd=always opensuseleap156-itt:latest "/usr/sbin/init"
   ```
